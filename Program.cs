...
// ZAVOLÁNÍ METODY PRO NAPLNÌNÍ DATABÁZE 
...
static async Task SeedDatabaseAsync(IHost host)
{
    using (var scope = host.Services.CreateScope())
    {
        ...
        try
        {
            ...
            // Pokud admin neexistuje, vytvoøíme ho
            if (adminUser == null)
            {
                adminUser = new IdentityUser { UserName = adminEmail, Email = adminEmail, EmailConfirmed = true };
                await userManager.CreateAsync(adminUser, adminPassword);
                logger.LogInformation("Admin úèet vytvoøen.");
            }
            // POKUD ADMIN EXISTUJE, ZKONTROLUJE A PØÍPADNÌ OPRAVÍ HESLO
            else
            {
            ...
            if (!await userManager.IsInRoleAsync(adminUser, UserRoles.Admin))
            {
                ...
                logger.LogInformation("Adminovi byla pøidìlena/potvrzena role 'Admin'.");
            }

            // Vytvoøení demo uživatelù
            if (!await dbContext.Client.AnyAsync())
            {
                var demoUsersData = new List<(IdentityUser User, string Password, Client ClientInfo)>
                {
                    ...
                    (new IdentityUser { UserName = "ron@weasley.cz", Email = "ron@weasley.cz", EmailConfirmed = true }, "Password.123", new Client { FirstName = "Ron", LastName = "Weasley", Email = "ron@weasley.cz", PhoneNumber = "987654321", Street = "Doupì 1", City = "Vydrník", Zip = "54321" }),
                ...
                {
                    new() { Type = "Pojištìní koštìte", Amount = 10000, StartDate = DateTime.Now, EndDate = DateTime.Now.AddYears(1), ClientId = clientsToSave[0].Id },
                    new() { Type = "Pojištìní auta", Amount = 20000, StartDate = DateTime.Now, EndDate = DateTime.Now.AddYears(6), ClientId = clientsToSave[0].Id },
                    new() { Type = "Pojištìní auta", Amount = 50000, StartDate = DateTime.Now, EndDate = DateTime.Now.AddYears(5), ClientId = clientsToSave[1].Id },
                    new() { Type = "Žitovní pojištìní", Amount = 250000, StartDate = DateTime.Now, EndDate = DateTime.Now.AddYears(10), ClientId = clientsToSave[2].Id },
                    new() { Type = "Žitovní pojištìní", Amount = 250000, StartDate = DateTime.Now, EndDate = DateTime.Now.AddYears(10), ClientId = clientsToSave[2].Id }
                };
                await dbContext.Insurance.AddRangeAsync(insurances);
                await dbContext.SaveChangesAsync();

                logger.LogInformation("Demo data byla úspìšnì vytvoøena.");
            }
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Nastala chyba pøi vytváøení úvodních dat.");
        }
    }
}